...
export default function RinkDetail() {
  ...
  useEffect(() => {
    ...
    const rinkSchema: any = {
      '@context': 'https://schema.org',
      '@type': 'LocalBusiness',
      name: rink.name,
      description: `Ice rink in ${rink.city}, ${rink.province_state || rink.country}`,
    ...
    if (rink.latitude && rink.longitude) {
      rinkSchema.geo = { '@type': 'GeoCoordinates', latitude: rink.latitude, longitude: rink.longitude };
    }
    if (rink.phone) rinkSchema.telephone = rink.phone;
    if (rink.website_url) rinkSchema.url = rink.website_url;
    if (rink.image_url) rinkSchema.image = rink.image_url;

    const script = document.createElement('script');
    script.type = 'application/ld+json';
    script.text = JSON.stringify([breadcrumbSchema, rinkSchema]);
          ...
          <dl style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            ...
            {rink.website_url && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                ...
                <dd>
                  <a href={rink.website_url} style={{ color: '#38bdf8', fontSize: '14px', textDecoration: 'none' }}>
                    {rink.website_url}
                  </a>
                </dd>
              </div>
