  ...
  try {
    ...
    if (data) {
      return {
        ...
        description: `${data.first_name} ${data.last_name} — ${data.position?.replace('_', ' ') || 'Hockey player'} for ${data.teams?.name || 'team'}. View stats, bio, and career information on RinkStop.`,
        openGraph: {
          title: `${data.first_name} ${data.last_name} | RinkStop`,
...
export default async function PlayerDetail({ params }: Props) {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: `${player.first_name} ${player.last_name}`, item: `https://rinkstop.com/directory/players/${player.slug}` },
    ],
  };
  ...
  const playerSchema = {
    ...
    url: `https://rinkstop.com/directory/players/${player.slug}`,
  ...
  // Build bio text
  const positionLabel = POSITIONS[player.position] || player.position?.replace('_', ' ') || 'hockey player';
  const teamInfo = player.teams?.name ? `playing for ${player.teams.name}` : '';
  const nationalityInfo = player.nationality ? `Representing ${player.nationality}` : '';
  const heightInfo = player.height_cm ? `Standing at ${player.height_cm}cm` : '';
  
  const bioParts = [player.first_name, player.last_name, 'is', 'a', positionLabel];
  if (teamInfo) bioParts.push(teamInfo);
  if (nationalityInfo) bioParts.push(nationalityInfo);
  if (heightInfo) bioParts.push(heightInfo);
  const bioText = bioParts.join(' ') + '. Follow their career and view full stats on RinkStop.';
    ...
    <>
      ...
      <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '0.75rem 1rem 3rem' }}>
        ...
        <Breadcrumbs links={[
          ...
          { label: `${player.first_name} ${player.last_name}`, href: `/directory/players/${player.slug}` },
        ]} />
        ...
        {/* Player Bio - descriptive content for SEO */}
        ...
        {stats && stats.length > 0 && (
          <div className="mb-8">
            <h2 style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: '1.125rem', color: '#fff', letterSpacing: '0.04em', marginBottom: '0.75rem' }}>SEASON STATS</h2>
            <div className="overflow-x-auto rounded-xl" style={{ background: 'var(--s2)', border: '1px solid var(--border)' }}>
              <table className="w-full text-sm">
                <thead>
                  <tr style={{ borderBottom: '1px solid var(--border)', color: 'rgba(255,255,255,0.4)' }}>
                ...
                <tbody>
                  {stats.map((stat: any) => (
                    <tr key={stat.id || stat.season} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                      <td className="px-4 py-3 text-white">{stat.season || 'N/A'}</td>
                      <td className="px-4 py-3 text-white">{stat.team_name || 'N/A'}</td>
                      <td className="px-4 py-3 text-center text-white/60">{stat.games_played || '0'}</td>
                      <td className="px-4 py-3 text-center text-white/60">{stat.goals || '0'}</td>
                      <td className="px-4 py-3 text-center text-white/60">{stat.assists || '0'}</td>
                      <td className="px-4 py-3 text-center text-white font-bold">{stat.goals + stat.assists || '0'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
