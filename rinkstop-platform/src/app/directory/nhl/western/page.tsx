...
function TeamCard({ t, bg }: { t: typeof CENTRAL_TEAMS[0]; bg: string }) {
  return (
    ...
    <main style={{ maxWidth: '1280px', margin: '0 auto', padding: '0.75rem 1rem 3rem' }}>
      ...
      <div style={{ marginBottom: '2rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: '0.625rem' }}>
          {CENTRAL_TEAMS.map(t => <TeamCard key={t.id} t={t} bg="#C8102E" />)}
        </div>
      </div>
      ...
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: '0.625rem' }}>
          {PACIFIC_TEAMS.map(t => <TeamCard key={t.id} t={t} bg="#1E5B9C" />)}
        </div>
      </div>
    </main>
  );
