...
// Only run main() when invoked directly (e.g. `node file.cjs`).
// When required as a module (e.g. from /api/cron/scores-refresh on Vercel),
// expose main() so the caller can await it.
