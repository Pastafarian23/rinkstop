...
export default function HighlightsGrid({
  ...
  // Article for the currently-open highlight. null = no published article yet.
  // Loaded when the user opens the popup, so the home page never pays the cost
  // for highlights that don't have a companion article.
  const [article, setArticle] = useState<{ slug: string; title: string; excerpt: string; subtitle?: string; og_image_url?: string; reading_time_minutes?: number; } | null>(null);
  ...
  // link in the popup. If not, the popup stays as it was.
  useEffect(() => {
    if (!selected) {
      setArticle(null);
      return;
    ...
    fetch(`/api/blog/posts?highlight_id=${selected.id}`)
      .then(r => r.json())
      .then(d => {
        if (cancelled) return;
      ...
      .catch(() => { if (!cancelled) setArticleLoading(false); });
      ...
      {selected && (
        ...
        >
          ...
          >
            ...
            {/* Companion article snippet. The article (if one exists) was
                generated by the article-from-highlight pipeline and linked
                to this highlight via posts.highlight_id. We only show this
                block when an article actually exists. */}
            {article && (
              <div
                style={{
                  marginTop: '1.25rem',
