...
export async function generateMetadata(): Promise<Metadata> {
  return { title: 'Hockey in China | RinkStop', description: 'China hockey: find ice rinks, teams, and leagues. China Women\'s Hockey League, fastest-growing market.', alternates: { canonical: 'https://rinkstop.com/directory/china' } };
}
export const dynamic = 'force-dynamic';
export default async function ChinaPage() {
  const [{ data: rinks }, { data: teams }, { count: rinkCount }] = await Promise.all([
    supabase.from('rinks').select('id, slug, name, city, address, phone, website_url').eq('country', 'China').eq('is_active', true).order('name').limit(50),
    supabase.from('teams').select('id, name, slug, logo_url').eq('country', 'China').eq('is_active', true).order('name').limit(20),
    supabase.from('rinks').select('*', { count: 'exact', head: true }).eq('country', 'China').eq('is_active', true),
  ]);
  ...
  return (
    <><div style={{ background: bg, color: textMain, minHeight: '100vh', fontFamily: 'system-ui, sans-serif' }}>
      <div style={{ borderBottom: `1px solid ${border}`, background: '#0f0f0f' }}><div style={{ maxWidth: 1280, margin: '0 auto', padding: '12px 24px' }}><nav style={{ fontSize: 13, color: textDim }}><a href="/" style={{ color: textDim, textDecoration: 'none' }}>Home</a><span style={{ margin: '0 6px' }}>›</span><a href="/directory" style={{ color: textDim, textDecoration: 'none' }}>Directory</a><span style={{ margin: '0 6px' }}>›</span><span style={{ color: textMuted }}>China</span></nav></div></div>
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '80px 24px', textAlign: 'center' }}><h1 style={{ fontFamily: "'Bebas Neue', Impact, sans-serif", fontSize: 'clamp(2.5rem, 8vw, 4rem)', color: textMain, letterSpacing: '0.04em', lineHeight: 1, marginBottom: 16 }}>HOCKEY IN CHINA</h1><p style={{ color: textMuted, fontSize: 16, maxWidth: 520, margin: '0 auto' }}>{rinkCount ?? 0} ice rinks. China Women's Hockey League, fastest-growing market.</p></div>
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 24px 80px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 16, marginBottom: 48 }}>
          ...
          <div style={{ background: card, border: `1px solid ${border}`, borderRadius: 12, padding: '20px', textAlign: 'center' }}><div style={{ fontSize: 32, fontWeight: 800, color: red }}>CWHL</div><div style={{ fontSize: 13, color: textMuted }}>Top League</div></div>
        </div>
        {teams && teams.length > 0 && (<div style={{ marginBottom: 48 }}><h2 style={{ fontFamily: "'Bebas Neue', Impact, sans-serif", fontSize: 28, letterSpacing: '0.04em', borderLeft: `4px solid ${red}`, paddingLeft: 16, marginBottom: 24, color: textMain }}>TEAMS IN CHINA</h2><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 12 }}>{teams.map(team => (<Link key={team.id} href={`/directory/teams/${team.slug || team.id}`} style={{ background: card, border: `1px solid ${border}`, borderRadius: 8, padding: '14px 16px', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 12 }}><span style={{ fontSize: 14, fontWeight: 600, color: textMain }}>{team.name}</span></Link>))}</div></div>)}
        {rinks && rinks.length > 0 && (<div style={{ marginBottom: 48 }}><h2 style={{ fontFamily: "'Bebas Neue', Impact, sans-serif", fontSize: 28, letterSpacing: '0.04em', borderLeft: `4px solid ${red}`, paddingLeft: 16, marginBottom: 24, color: textMain }}>ICE RINKS IN CHINA</h2><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 16 }}>{rinks.map(rink => (<div key={rink.id} style={{ background: card, border: `1px solid ${border}`, borderRadius: 10, padding: 20 }}><h3 style={{ fontSize: 15, fontWeight: 700, color: textMain, marginBottom: 4 }}>{rink.name}</h3><div style={{ fontSize: 13, color: textMuted, marginBottom: 8 }}>{rink.city ?? ''}</div>{rink.address && <div style={{ fontSize: 12, color: textDim, marginBottom: 4 }}>{rink.address}</div>}{rink.phone && <div style={{ fontSize: 12, color: textDim, marginBottom: 4 }}>📞 {rink.phone}</div>}{rink.website_url && <a href={rink.website_url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 12, color: red, textDecoration: 'none' }}>🌐 Website →</a>}</div>))}</div></div>)}
