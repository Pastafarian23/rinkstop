...
export async function generateMetadata(): Promise<Metadata> {
  return {
    title: 'Hockey in France | RinkStop',
    description: 'France hockey: find ice rinks, teams, and leagues. French hockey with growing NHL presence.',
    alternates: { canonical: 'https://rinkstop.com/directory/france' },
  };
}
...
export default async function FrancePage() {
  const [{ data: rinks }, { data: teams }, { count: rinkCount }] = await Promise.all([
    supabase.from('rinks').select('id, slug, name, city, address, phone, website_url').eq('country', 'France').eq('is_active', true).order('name').limit(50),
    supabase.from('teams').select('id, name, slug, logo_url').eq('country', 'France').eq('is_active', true).order('name').limit(20),
    supabase.from('rinks').select('*', { count: 'exact', head: true }).eq('country', 'France').eq('is_active', true),
  ]);
      ...
      <div style={{ background: bg, color: textMain, minHeight: '100vh', fontFamily: 'system-ui, sans-serif' }}>
        <div style={{ borderBottom: `1px solid ${border}`, background: '#0f0f0f' }}>
          <div style={{ maxWidth: 1280, margin: '0 auto', padding: '12px 24px' }}>
            <nav style={{ fontSize: 13, color: textDim }}>
              ...
              <span style={{ margin: '0 6px' }}>›</span>
              <span style={{ color: textMuted }}>France</span>
            </nav>
          </div>
        </div>
        ...
        <div style={{ maxWidth: 1280, margin: '0 auto', padding: '80px 24px', textAlign: 'center' }}>
          ...
          <p style={{ color: textMuted, fontSize: 16, maxWidth: 520, margin: '0 auto' }}>
            {rinkCount ?? 0} ice rinks. French hockey with growing NHL presence.
          </p>
        </div>
        ...
        <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 24px 80px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 16, marginBottom: 48 }}>
            ...
            <div style={{ background: card, border: `1px solid ${border}`, borderRadius: 12, padding: '20px', textAlign: 'center' }}>
              <div style={{ fontSize: 32, fontWeight: 800, color: red }}>Ligue Magnus</div>
