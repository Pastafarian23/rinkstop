...
export default function YouthProgramsClient({ initialPrograms, initialCountry }: Props) {
  ...
  return (
    <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 1rem 4rem' }}>
      ...
      <div style={{
        ...
        {/* Country select — server-rendered, navigates with searchParams */}
        <select
          value={initialCountry}
          onChange={e => {
            const url = e.target.value
              ? `/directory/youth-hockey/programs?country=${encodeURIComponent(e.target.value)}`
              : '/directory/youth-hockey/programs';
            window.location.href = url;
          }}
          style={{
            background: 'var(--s2)', border: '1px solid var(--border)',
            color: initialCountry ? '#fff' : 'var(--muted)',
        ...
        {(search || initialCountry) && (
          <button
            onClick={() => { setSearch(''); window.location.href = '/directory/youth-hockey/programs'; }}
            style={{
      ...
      <p style={{ fontSize: '0.75rem', color: '#555555', letterSpacing: '0.04em', marginBottom: '1rem' }}>
        ...
        {initialCountry ? ` in ${initialCountry}` : ''}
        ...
        {filtered.length === 0 ? (
          <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '3rem 1rem' }}>
            <p style={{ color: 'rgba(255,255,255,0.3)', marginBottom: '1rem' }}>
              {initialCountry ? `No programs listed in ${initialCountry} yet.` : 'No programs found matching your search.'}
            </p>
