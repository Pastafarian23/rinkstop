...
// Accepts a pending connection request. Only the recipient can accept.
export async function POST(
  ...
  const result = await checkRateLimit(`accept:${ip}`, RL);
  maybeCleanup();
  ...
  if (!isRecipient) {
    return NextResponse.json(
      { error: 'Only the recipient of a pending request can accept it.' },
      { status: 403 }
    );
  }
  if (connection.status !== 'pending') {
    return NextResponse.json({ error: `Cannot accept a ${connection.status} connection.` }, { status: 400 });
  }
  ...
  if (error) {
    console.error('[connections accept] update failed', error);
