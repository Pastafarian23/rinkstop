...
export async function GET(req: NextRequest, { params }: RouteParams) {
  ...
  if (type === 'news') {
    const { data } = await baseQuery(
      supabaseAdmin.from('team_news').select('id, title, is_published, published_at, created_at')
    );
    return NextResponse.json({ data: data || [] });
  }
  if (type === 'results') {
    const { data } = await baseQuery(
      supabaseAdmin.from('team_results').select('id, game_date, opponent, our_score, their_score, outcome, created_at')
    );
    return NextResponse.json({ data: data || [] });
  }
  if (type === 'schedule') {
    const { data } = await baseQuery(
      supabaseAdmin
        .from('team_schedule')
        .select('id, scheduled_at, opponent, kind, venue, home_away, is_cancelled, created_at')
    );
    return NextResponse.json({ data: data || [] });
  }
  ...
  const [news, results, schedule] = await Promise.all([
    baseQuery(supabaseAdmin.from('team_news').select('id, title, published_at, is_published, created_at')),
    baseQuery(supabaseAdmin.from('team_results').select('id, game_date, opponent, our_score, their_score, created_at')),
    baseQuery(supabaseAdmin.from('team_schedule').select('id, scheduled_at, opponent, kind, is_cancelled, created_at')),
  ]);
