// src/app/api/referee/credentials/withdraw/route.ts
// POST /api/referee/credentials/withdraw
//
// Referee withdraws a pending|rejected federation_registrations row.
...
export async function POST(request: NextRequest) {
  ...
  const result = await checkRateLimit(`ref-creds-withdraw:${ip}`, RATE_LIMIT);
  maybeCleanup();
  if (!result.allowed) {
