// src/app/dashboard/manage/rink/[id]/programming/new/page.tsx
//
// WS17 PR3a - Owner programming create form.
//
// Client component. Submits to POST /api/owner/rinks/[id]/programming.
// Redirects to /dashboard/manage/rink/[id]/programming on success.
...
export default function NewProgrammingPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch(`/api/owner/rinks/${id}/programming`, {
      method: 'POST',
  ...
  return (
    <div style={{ maxWidth: 720 }}>
      ...
      <h1 style={{ fontSize: '1.6rem', fontWeight: 700, color: '#fff', marginTop: '0.5rem' }}>Add programming</h1>
      <p style={{ fontSize: '0.9rem', color: '#94A3B8', marginTop: '0.25rem' }}>
        Recurring weekly slot. Visible to visitors on your rink page once published.
      </p>
      ...
      <form onSubmit={handleSubmit} style={{ marginTop: '1.5rem', background: 'rgba(13,17,23,0.6)', border: '1px solid var(--border)', borderRadius: 12, padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        ...
        <Field label="Gear rules (optional)">
          <textarea value={form.gear_rules} onChange={e => update('gear_rules', e.target.value)} rows={2} style={{ ...inputStyle, resize: 'vertical' }} placeholder="e.g. Full hockey gear required. Helmets available for rent." />
        </Field>
        ...
        <div style={{ display: 'flex', gap: '0.75rem', marginTop: '0.5rem' }}>
          <button type="submit" disabled={submitting} style={{ background: '#38BDF8', color: '#0F172A', border: 'none', borderRadius: 8, padding: '0.625rem 1.25rem', fontWeight: 600, fontSize: '0.875rem', cursor: submitting ? 'wait' : 'pointer' }}>
            {submitting ? 'Saving…' : form.status === 'published' ? 'Publish' : 'Save draft'}
          </button>
