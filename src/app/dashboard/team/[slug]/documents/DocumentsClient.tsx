...
interface Props {
  ...
  userId: string;
  isAdmin: boolean;
  roster: RosterEntry[];     // A-i: recipient picker
  managedKids: ManagedKid[]; // A-ii: child picker
  documents: Doc[];
}
...
export default function DocumentsClient({ teamId, teamSlug, teamName, userId, isAdmin, roster, managedKids, documents }: Props) {
  ...
  async function handleUpload(e: React.FormEvent) {
    ...
    try {
      ...
      setRecipientUserIds(new Set()); // A-i: reset recipient picker on success
      setBroadcast(true);
      setUploading(false);
      router.refresh();
    } catch (err) {
  ...
  async function handleSign() {
    ...
    try {
      ...
      // A-ii: include player_id only when the parent/guardian picked a kid.
