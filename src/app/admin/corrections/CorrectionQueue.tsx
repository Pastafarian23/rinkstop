...
export default function CorrectionQueue({ pending, recent, submitterLabels, entityLabels }: Props) {
  ...
  async function handleAction(id: string, action: 'approve' | 'reject') {
    ...
    setError(null);
    try {
      const r = await fetch(`/api/corrections/${id}/review`, {
        method: 'POST',
      ...
      >
        ...
        {pending.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            {pending.map((c) => {
              const entityLabel = entityLabels[`${c.entity_type}:${c.entity_id}`] || c.entity_id.slice(0, 12);
              const submitterLabel = submitterLabels[c.submitter_user_id] || c.submitter_user_id.slice(0, 12);
              return (
                <div
                  key={c.id}
                  data-testid={`correction-pending-${c.id}`}
                  style={{
                    background: '#0a0a0a',
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8, marginBottom: '0.6rem' }}>
                    <div style={{ color: '#fff', fontSize: '0.95rem', fontWeight: 700 }}>
                      {c.entity_type}: {entityLabel} · <span style={{ color: 'rgba(255,255,255,0.65)' }}>{c.field_name}</span>
                    </div>
                    <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.75rem' }}>
                      by {submitterLabel} · {new Date(c.submitted_at).toLocaleString()}
                    </div>
                  </div>

                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', marginBottom: '0.6rem' }}>
                    <div style={{ background: '#0f0f0f', border: '1px solid #1e1e1e', borderRadius: 6, padding: '0.4rem 0.6rem' }}>
                      <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.65rem', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 2 }}>Current</div>
                      <div style={{ color: 'rgba(255,255,255,0.7)', fontSize: '0.85rem', wordBreak: 'break-word' }}>{c.current_value || '—'}</div>
                    </div>
                    <div style={{ background: 'rgba(20,184,166,0.06)', border: '1px solid rgba(20,184,166,0.3)', borderRadius: 6, padding: '0.4rem 0.6rem' }}>
                      <div style={{ color: '#14B8A6', fontSize: '0.65rem', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 2 }}>Proposed</div>
                      <div style={{ color: '#fff', fontSize: '0.85rem', wordBreak: 'break-word' }}>{c.proposed_value}</div>
                    </div>
                  </div>

                  <div style={{ color: 'rgba(255,255,255,0.55)', fontSize: '0.85rem', lineHeight: 1.5, marginBottom: '0.75rem' }}>
                ...
                >
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ color: 'rgba(255,255,255,0.85)', fontSize: '0.85rem' }}>
                      {c.entity_type}: {entityLabel} · {c.field_name}
                    </div>
