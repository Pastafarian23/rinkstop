...
export default function EditPlayer() {
  ...
  const [form, setForm] = useState({ first_name:'', last_name:'', slug:'', team_id:'', position:'forward', jersey_number:0, shoots:'', catches:'', height_cm:0, weight_kg:0, nationality:'', bio:'', headshot_url:'' });

  useEffect(() => {
    fetch('/api/teams').then(r => r.json()).then(d => setTeams(d.data || []));
    fetch(`/api/players?id=${id}`).then(r => r.json()).then(d => { if (d.data?.[0]) setForm(d.data[0]); });
  }, [id]);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    await fetch(`/api/players?id=${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) });
