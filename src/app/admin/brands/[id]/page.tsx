...
export default function EditBrand() {
  ...
  useEffect(() => {
    fetch(`/api/brands?id=${id}`).then(r => r.json()).then(d => { if (d[0]) setForm(d[0]); });
  }, [id]);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    await fetch('/api/brands', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id, ...form }) });
