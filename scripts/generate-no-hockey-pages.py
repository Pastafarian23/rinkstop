...
base = '/root/.openclaw/workspace/rinkstop-platform/src/app/directory'
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))

# Template uses [[{{}}]] as placeholder for {} to avoid f-string conflicts
METADATA_TEMPLATE = """import type { Metadata } from 'next';
      ...
      <div style={{ textAlign: 'center', padding: '4rem 2rem' }}>
        ...
        <div style={{ background: 'var(--s2)', border: '1px solid var(--border)', borderRadius: '8px', padding: '2rem', maxWidth: '500px', margin: '0 auto', textAlign: 'left' }}>
          <h2 style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: '1.2rem', letterSpacing: '0.04em', color: 'rgba(255,255,255,0.5)', marginBottom: '1rem' }}>
            WANT TO HELP GROW HOCKEY IN [[COUNTRY_UPPER]]?
          </h2>
...
def fill(template, country, flag, region, slug, funcname):
    return (template
        .replace('[[COUNTRY]]', country)
        .replace('[[COUNTRY_UPPER]]', country.upper())
        .replace('[[FLAG]]', flag)
        .replace('[[REGION]]', region)
        .replace('[[SLUG]]', slug)
        .replace('[[FUNCNAME]]', funcname))

for country, flag, region in countries:
    slug = country.lower().replace(' ', '-').replace('&', 'and')
    funcname = ''.join(w.capitalize() for w in slug.split('-'))
    dirpath = os.path.join(base, slug)
    os.makedirs(dirpath, exist_ok=True)

    with open(os.path.join(dirpath, 'metadata.ts'), 'w') as f:
        f.write(fill(METADATA_TEMPLATE, country, flag, region, slug, funcname))

    with open(os.path.join(dirpath, 'page.tsx'), 'w') as f:
        f.write(fill(PAGE_TEMPLATE, country, flag, region, slug, funcname))

    print(f"✅ {country} -> /directory/{slug}")

print(f"\n🎉 Done! {len(countries)} pages generated in {base}")