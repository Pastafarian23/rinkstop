# HEARTBEAT.md

# Daily Health Check (Ron checks each heartbeat)
- [ ] Check cron job status (`openclaw cron list`) - verify no errors
- [ ] Check last delivery status for each project channel

# Daily CEO Checklist (to post in CEO channel each morning)

## Channel Reference
- **CEO Channel:** Arnel's CEO (this chat)
- **Social Media:** Social Media Agents
- **Blog Posts:** Blog Posts channel

## Daily Tasks for Arnel
- [ ] Review & approve social posts (see Social Media channel)
- [ ] Review & approve blog posts (see Blog Posts channel)
- [ ] Any other pending items?

## Workflow
1. Social agents → Social Media channel for drafting
2. Content agents → Blog Posts channel for drafting
3. Ready posts → CEO channel for approval
4. After approval → Ron posts → updates tracker

## Instructions
- Check both channels for proposed content
- React with ✅ to approve, ❌ to reject
- If no response by EOD → Ron sends reminder in CEO chat